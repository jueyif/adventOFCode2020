...
limit = re.findall(r'\d{1,2}', input)
key = re.findall(r'\w\:', input)
password = re.findall(r'\w{3,99}', input)
...
for i in range(length):
    count = 0
    for letter in password[i]:
        if key[i].strip(':') == letter:
            count = count+1
    if count >= int(limit[2*i]) and count <= int(limit[2*i+1]):
        correctPassword.append(password[i])

print('Part 1:', len(correctPassword))
