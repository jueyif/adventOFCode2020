...
for i in range(length):
    subInput = input[i+1:]
    for j in range(len(subInput)):
        for k in subInput[j+1:]:
            if input[i] + subInput[j] + k == 2020:
                print('input[i]', input[i], 'subInput[j]', subInput[j], 'k', k)
                print(input[i] + subInput[j] + k)
                print(input[i] * subInput[j] * k)
                result.append(input[i] * subInput[j] * k)
...
#         for k in q[j+1:]:
#                 print('qq[i]',qq[i],'q[j]', q[j], 'k', k)
#                 print(qq[i] + qq[j+1] + k)
#                 result.append(qq[i] + qq[j+1] + k)
